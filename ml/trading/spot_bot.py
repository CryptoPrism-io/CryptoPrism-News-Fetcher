...
Usage:
    python -m ml.trading.spot_bot --run          # single cycle: longs + shorts + close expired
    python -m ml.trading.spot_bot --close-all    # emergency: close everything
    python -m ml.trading.spot_bot --status        # show portfolio + P&L
"""
...
from crypto_db import get_db_conn
from ml.trading.futures_exchange import (
...
from ml.trading.spot_exchange import build_exchange, get_price
...
def sync_db_with_exchange(conn, exchange) -> int:
    ...
    """
    from ml.trading.futures_exchange import get_open_futures_positions
...
def run_signal_cycle():
    ...
    from ml.models.registry import get_active_model
    ...
    from ml.models.regime import get_current_regime
    ...
    try:
        from ml.trading.daily_report import send_telegram
